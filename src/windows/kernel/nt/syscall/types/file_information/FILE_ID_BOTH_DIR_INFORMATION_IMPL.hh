...
class FILE_ID_BOTH_DIR_INFORMATION_ENTRY_IMPL : public FILE_ID_BOTH_DIR_INFORMATION_ENTRY {
  public:
    ...
    uint64_t FileId() const override { return data_->FileId; }
    void FileId(uint64_t FileId) override { data_->FileId = FileId; }

    GuestVirtualAddress address() const override { return gva_; }
    uint32_t buffer_size() const override { return FileNameOffset + data_->FileNameLength; }

    FILE_ID_BOTH_DIR_INFORMATION_ENTRY_IMPL(const GuestVirtualAddress& gva)
        : gva_(gva), data_(gva),
  ...
  private:
    ...
    guest_ptr<structs::_FILE_ID_BOTH_DIR_INFORMATION> data_;
    WStr ShortName_;
    WStr FileName_;
};
...
class FILE_ID_BOTH_DIR_INFORMATION_IMPL final
  ...
  public:
    ...
    FILE_ID_BOTH_DIR_INFORMATION_IMPL(const GuestVirtualAddress& gva, uint32_t buffer_size);
