...
void FILE_NETWORK_OPEN_INFORMATION_IMPL::write(std::ostream& os,
    ...
    os << linePrefix << "EndOfFile:      0x" << std::hex << EndOfFile() << std::dec << '\n';
    os << linePrefix << "Attributes:     " << FileAttributes().dir_string() << '\n';
}

Json::Value FILE_NETWORK_OPEN_INFORMATION_IMPL::json() const {
    Json::Value result;
