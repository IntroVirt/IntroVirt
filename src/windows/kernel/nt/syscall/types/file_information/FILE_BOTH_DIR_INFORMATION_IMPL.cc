...
FILE_BOTH_DIR_INFORMATION_IMPL::FILE_BOTH_DIR_INFORMATION_IMPL(const guest_ptr<void>& ptr,
                                                               uint32_t buffer_size)
    : offset_iterable(
          [](const guest_ptr<void>& ptr, uint32_t buffer_size) {
              return std::make_shared<FILE_BOTH_DIR_INFORMATION_ENTRY_IMPL>(ptr);
          },
          ptr, buffer_size) {
...
FILE_BOTH_DIR_INFORMATION_ENTRY::make_shared(const guest_ptr<void>& ptr) {
    return std::make_shared<FILE_BOTH_DIR_INFORMATION_ENTRY_IMPL>(ptr);
}
