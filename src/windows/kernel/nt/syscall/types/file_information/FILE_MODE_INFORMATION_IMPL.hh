...
class FILE_MODE_INFORMATION_IMPL final : public FILE_MODE_INFORMATION {
  public:
    uint32_t Mode() const override { return data_->Mode; }
    void Mode(uint32_t value) override { data_->Mode = value; }
    ...
    FILE_MODE_INFORMATION_IMPL(const GuestVirtualAddress& gva, uint32_t buffer_size);

  private:
    const GuestVirtualAddress gva_;
    const uint32_t buffer_size_;
    guest_ptr<structs::_FILE_MODE_INFORMATION> data_;
};
