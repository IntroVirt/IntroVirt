...
class SECTION_IMPL final : public OBJECT_IMPL<PtrType, SECTION> {
  public:
    ...
    SECTION_IMPL(const NtKernelImpl<PtrType>& kernel, const GuestVirtualAddress& gva);
    SECTION_IMPL(const NtKernelImpl<PtrType>& kernel,
                 std::unique_ptr<OBJECT_HEADER_IMPL<PtrType>>&& object_header);

  private:
    const NtKernelImpl<PtrType>& kernel_;

    const structs::SECTION* section;
