...
static log4cxx::LoggerPtr logger(log4cxx::Logger::getLogger("introvirt.windows.SystemCall"));

static const std::string SystemCallToStringMap[] = {
    // Nt
    "NtAcceptConnectPort",
    "NtAccessCheck",
...
const std::string& to_string(SystemCallIndex index) {
    static const std::string BUG_STR("BUG! Call missing from map");
    static const std::string UNKNOWN_STR("Unknown System Call");

    if (likely(index <= SystemCallIndex::SYSTEM_CALL_MAX)) {
        return SystemCallToStringMap[static_cast<uint32_t>(index)];
    } else if (index == SystemCallIndex::UNKNOWN_SYSTEM_CALL) {
        return UNKNOWN_STR;
    }

    LOG4CXX_ERROR(logger, "Missing SystemCallIndex " << static_cast<uint>(index));
    return BUG_STR;
}

std::ostream& operator<<(std::ostream& os, const SystemCallIndex index) {
    os << to_string(index);
    return os;
}

SystemCallIndex system_call_from_string(std::string name) {
    static std::unordered_map<std::string, SystemCallIndex> StringToSystemCallMap;
    if (unlikely(StringToSystemCallMap.empty())) {
        // Build up the map if we haven't done so already
        for (uint i = 0; i <= static_cast<uint>(SystemCallIndex::SYSTEM_CALL_MAX); ++i) {
            StringToSystemCallMap[boost::algorithm::to_lower_copy(SystemCallToStringMap[i])] =
                static_cast<SystemCallIndex>(i);
        }
    }
    ...
    // Replace leading "zw" with "nt" if it exists
    if (unlikely(boost::algorithm::starts_with(name, "zw"))) {
        name[0] = 'n';
        name[1] = 't';
    }

    // Look for the given entry
    auto iter = StringToSystemCallMap.find(name);
    if (unlikely(iter == StringToSystemCallMap.end()))
