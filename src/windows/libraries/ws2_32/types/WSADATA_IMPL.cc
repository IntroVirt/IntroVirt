...
std::shared_ptr<WSADATA> WSADATA::make_shared(const guest_ptr<void>& ptr, bool x64) {
    if (x64) {
        return std::make_shared<WSADATA_IMPL<uint64_t>>(ptr);
    }
    return std::make_shared<WSADATA_IMPL<uint32_t>>(ptr);
}
