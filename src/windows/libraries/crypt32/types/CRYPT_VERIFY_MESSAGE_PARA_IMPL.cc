...
#include "CRYPT_VERIFY_MESSAGE_PARA_IMPL.hh"
...
size_t CRYPT_VERIFY_MESSAGE_PARA::size(bool x64) {
    if (x64) {
        return sizeof(structs::_CRYPT_VERIFY_MESSAGE_PARA<uint64_t>);
    }
    return sizeof(structs::_CRYPT_VERIFY_MESSAGE_PARA<uint32_t>);
}

size_t CRYPT_VERIFY_MESSAGE_PARA::size(const Vcpu& vcpu) {
    return CRYPT_VERIFY_MESSAGE_PARA::size(vcpu.long_mode() && !vcpu.long_compatibility_mode());
}
