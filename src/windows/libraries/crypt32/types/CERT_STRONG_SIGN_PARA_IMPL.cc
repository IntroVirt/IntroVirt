...
std::shared_ptr<CERT_STRONG_SIGN_PARA>
CERT_STRONG_SIGN_PARA::make_shared(const guest_ptr<void>& ptr, bool x64) {
    if (x64) {
        return std::make_shared<CERT_STRONG_SIGN_PARA_IMPL<uint64_t>>(ptr);
    }
    return std::make_shared<CERT_STRONG_SIGN_PARA_IMPL<uint32_t>>(ptr);
}
