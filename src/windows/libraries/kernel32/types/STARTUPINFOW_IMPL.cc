...
std::shared_ptr<STARTUPINFOW> STARTUPINFOW::make_shared(const guest_ptr<void>& ptr, bool x64) {
    if (x64) {
        return std::make_shared<STARTUPINFOW_IMPL<uint64_t>>(ptr);
    } else {
        return std::make_shared<STARTUPINFOW_IMPL<uint32_t>>(ptr);
    }
}
...
GuestAllocation<windows::kernel32::STARTUPINFOW>::GuestAllocation() {
    ...
    const size_t structure_size =
        (x64) ? sizeof(structs::_STARTUPINFOW<uint64_t>) : sizeof(structs::_STARTUPINFOW<uint32_t>);
    ...
    value_ = STARTUPINFOW::make_shared(ptr, x64);

    // The _STARTUPINFOW struct's cb member is the "count of bytes".
