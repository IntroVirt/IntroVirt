...
#include <introvirt/windows/libraries/kernel32/types/STARTUPINFOA.hh>
...
struct _STARTUPINFOA {
    uint32_t cb;
    guest_member_ptr<char[], PtrType> lpReserved;
    guest_member_ptr<char[], PtrType> lpDesktop;
    guest_member_ptr<char[], PtrType> lpTitle;
    uint32_t dwX;
    uint32_t dwY;
    uint32_t dwXSize;
    uint32_t dwYSize;
...
template <typename PtrType>
class STARTUPINFOA_IMPL final : public STARTUPINFOA {
  public:
    uint32_t cb() const override { return ptr_->cb; }
    void cb(uint32_t cb) override { ptr_->cb = cb; }

    guest_ptr<char[]> lpReserved() const override { return ptr_->lpReserved.cstring(ptr_); }
    void lpReserved(const guest_ptr<char[]>& lpReserved) override {
        ptr_->lpReserved.set(lpReserved);
    }

    guest_ptr<char[]> lpDesktop() const override { return ptr_->lpDesktop.cstring(ptr_); }
    void lpDesktop(const guest_ptr<char[]>& lpDesktop) override { ptr_->lpDesktop.set(lpDesktop); }

    guest_ptr<char[]> lpTitle() const override { return ptr_->lpTitle.cstring(ptr_); }
    void lpTitle(const guest_ptr<char[]>& lpTitle) override { ptr_->lpTitle.set(lpTitle); }
    ...
    STARTUPINFOA_IMPL(const guest_ptr<void>& ptr) : ptr_(ptr) {}

    ~STARTUPINFOA_IMPL() override = default;

  private:
    guest_ptr<structs::_STARTUPINFOA<PtrType>> ptr_;
};
