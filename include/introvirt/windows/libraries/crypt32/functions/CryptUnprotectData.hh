 ...
 */
class CryptUnprotectData final : public WindowsFunctionCall {
  public:
    /* Input arguments */
    GuestVirtualAddress pDataIn() const;
    void pDataIn(const GuestVirtualAddress& value);

    GuestVirtualAddress ppszDataDescr() const;
    void ppszDataDescr(const GuestVirtualAddress& value);
    ...
    CryptUnprotectData(Event& event);
    ~CryptUnprotectData() override;

    static constexpr int ArgumentCount = 7;
    inline static const std::string LibraryName = "crypt32";
    inline static const std::string FunctionName = "CryptUnprotectData";

  private:
    GuestVirtualAddress pDataIn_;
    GuestVirtualAddress ppszDataDescr_;
